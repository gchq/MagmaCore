 ...
 */
public class MagmaCoreJenaDatabase implements ObjectDatabase {
    ...
    public void create(final Thing object) {
        ...
        object.getPredicates().forEach((iri, predicates) -> predicates.forEach(predicate -> resource
                .addProperty(dataset.getDefaultModel().createProperty(iri.toString()),
                        predicate.toString())));
    }
    ...
    public List<Thing> findByPredicateIri(final IRI iri, final IRI object) {
        final String query =
                "SELECT ?s ?p ?o WHERE {?s ?p ?o. ?s <" + iri.toString() + "> <" + object.toString()
                        + ">.}";
        final QueryResultList list = executeQuery(query);
     ...
     */
    @Override
    public Thing findByPredicateIriAndStringCaseInsensitive(final IRI predicateIri,
            final String value) {
        final String query =
                "SELECT ?s ?p ?o WHERE {{ SELECT ?s ?p ?o where { ?s ?p ?o.}}{select ?s where {?s <"
